---
name: sales-prep
description: Meeting preparation brief covering attendee intelligence, talking points, discovery questions, expected objections, and success metrics. Use when user mentions meeting prep, briefing, sales call preparation, demo prep, or asks to prepare for a specific meeting/call with a prospect. Triggers on "Meeting Vorbereitung", "Briefing", "Demo Vorbereitung", "auf Call vorbereiten", "Pitch prep", "Discovery Call vorbereiten".
---
...
Before generating the meeting brief, you **must** load the structural template. The user may have customized this template — check for a user-override first:

1. Check if a user override exists at `~/.claude/ki-vertriebsteam/templates/meeting-prep.md`. If yes, `Read` that file.
2. If no override exists, `Read` the plugin template at `${CLAUDE_PLUGIN_ROOT}/skills/sales-prep/templates/meeting-prep.md`.
3. Use the loaded template as your structural anchor: follow its section order, replace every `[PLACEHOLDER]` token with prospect-specific content. The template's structure overrides any structural examples in the skill body — this skill body is written for maintainer clarity, the template defines the actual output shape.
...
```
/ki-vertriebsteam:sales-prep <url>
```
