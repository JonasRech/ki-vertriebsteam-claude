...
description: Pipeline report rendered as PDF via reportlab — cover page, score charts, top-prospect detail pages, action plan, methodology. Use when user mentions PDF report, pipeline PDF, executive report for stakeholders, printable sales summary, board-ready report. Triggers on "PDF Report", "Pipeline PDF", "Bericht als PDF", "Executive Report", "Reporting für Geschäftsführung".
---
...
- **Invocation:** `/ki-vertriebsteam:sales-report-pdf`
...
When the user invokes `/ki-vertriebsteam:sales-report-pdf`, follow this process:
...
- Inform the user: "No SALES-REPORT.md found. Run `/ki-vertriebsteam:sales-report` first to generate the pipeline report, then run `/ki-vertriebsteam:sales-report-pdf` to create the PDF version."
- Stop execution.
...
### Step 5: Verify PDF Generation Script Availability

The PDF generation script ships with the plugin at `${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py`. The `${CLAUDE_PLUGIN_ROOT}` environment variable is resolved automatically at runtime to the plugin's install directory — this works regardless of which working directory the user invoked Claude Code from.

**Verify the script exists:**

```bash
test -f "${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py" && echo "OK" || echo "MISSING"
```

**If the script does NOT exist (rare — would indicate a corrupted plugin install):**
- Inform the user: "The PDF generation script was not found at `${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py`. The plugin install may be corrupted — try reinstalling: `/plugin install ki-vertriebsteam@rech-studio`."
- Stop execution.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py" _pdf_input.json "SALES-REPORT-$(date +%Y-%m-%d).pdf"
```
...
### Script Not Found
```
The PDF generation script was not found at ${CLAUDE_PLUGIN_ROOT}/scripts/generate_pdf_report.py.
This indicates a corrupted plugin install. Try reinstalling:
...
Run `/ki-vertriebsteam:sales-report` first to generate the pipeline report, then run
`/ki-vertriebsteam:sales-report-pdf` to create the PDF version.
```
