---
name: sales-icp
description: Ideal Customer Profile (ICP) builder producing structured target-segment definition with firmographic, technographic, and behavioral criteria. Use when user mentions ICP, ideal customer, target audience definition, customer profiling, lookalike accounts, or wants to define who they sell to. Triggers on "ICP", "Wunschkunde", "Zielgruppe definieren", "ideal customer profile", "Lookalike", "Zielkundenprofil".
---
...
- **Invocation:** `/ki-vertriebsteam:sales-icp <description>`
...
When the user invokes `/ki-vertriebsteam:sales-icp <description>`, follow this process:
...
- **Feedback Loop:** After running `/ki-vertriebsteam:sales-prospect` on 10+ companies, review which scores correlated with actual deal outcomes. Adjust ICP criteria and scoring weights accordingly.
...
7. After writing, give the user a brief summary of the ICP highlights and suggest next steps (e.g., "Run `/ki-vertriebsteam:sales-prospect <url>` to analyze a specific company against this ICP").
