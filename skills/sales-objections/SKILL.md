...
description: Objection handling playbook generator with LAER framework (Listen, Acknowledge, Explore, Respond) covering 15 universal + competitive + pricing objections. Use when user mentions objections, einwände, pushback, "too expensive", "no budget", "wrong time", competitor objections, or asks how to respond to common sales pushbacks. Triggers on "Einwand", "Einwandbehandlung", "objection", "Gegenargument", "Pushback", "LAER".
---
...
Before generating the objection playbook, you **must** load the structural template. The user may have customized this template — check for a user-override first:

1. Check if a user override exists at `~/.claude/ki-vertriebsteam/templates/objection-playbook.md`. If yes, `Read` that file.
2. If no override exists, `Read` the plugin template at `${CLAUDE_PLUGIN_ROOT}/skills/sales-objections/templates/objection-playbook.md`.
3. Use the loaded template as your structural anchor: follow its section order, the LAER framework structure (Acknowledge / Explore / Respond / Redirect), replace every `[PLACEHOLDER]` with prospect-specific content. The template's structure overrides any structural examples in the skill body.
...
```
/ki-vertriebsteam:sales-objections <topic/industry>
```
