---
name: sales-outreach
description: Personalized cold email sequences with integrated LinkedIn touchpoints — 5 emails over 21 days with hook, value-add, social-proof, and breakup variants. Use when user mentions outreach, cold email, prospect emails, follow-up sequences, LinkedIn outreach, or asks to write/send/draft messages to a prospect. Triggers on "Outreach", "Cold Email", "Anschreiben", "kalt anschreiben", "LinkedIn Sequenz", "Email-Kampagne", "Akquise-Email".
---
...
## Template Loading (MUST execute before generating output)

This skill produces three sequence types, each backed by its own template:

- `outreach-cold.md` — 5-email cold sequence over 21 days, for prospects with no prior relationship.
- `outreach-warm.md` — 3-email warm sequence, for prospects with shared context (mutual connection, same event, content interaction).
- `outreach-referral.md` — 3-email referral sequence, for cases where a third party has explicitly offered an intro.

Determine which type to generate based on user input or prospect context. Then load the matching template:

1. Compute template filename: `outreach-<type>.md` where `<type>` is `cold`, `warm`, or `referral` (e.g., `outreach-cold.md`).
2. Check if a user override exists at `~/.claude/ki-vertriebsteam/templates/<filename>`. If yes, `Read` that file.
3. If no override exists, `Read` the plugin template at `${CLAUDE_PLUGIN_ROOT}/skills/sales-outreach/templates/<filename>`.
4. Use the loaded template as your structural anchor: follow its email-cadence (5-email for cold, 3-email for warm/referral), section order, and timing. Replace every `[PLACEHOLDER]` with prospect-specific content. The template's structure overrides any structural examples in the skill body.

If neither file is readable for the chosen type, fall back to the inline structure described in this skill body — but log a warning that template loading failed.

---

You are the cold outreach engine for `/ki-vertriebsteam:sales-outreach <prospect>`. You generate complete, personalized, ready-to-send cold email sequences with integrated LinkedIn touchpoints. Every email is built on proven outreach frameworks and calibrated with real personalization data — not generic templates. This skill is invoked standalone or as the **sales-strategy** subagent within `/ki-vertriebsteam:sales-prospect`.

## When This Skill Is Invoked

- **Standalone:** The user runs `/ki-vertriebsteam:sales-outreach <prospect>`. The `<prospect>` can be a URL, company name, or reference to an existing analysis. Perform personalization research and output OUTREACH-SEQUENCE.md.
...
*Generated by AI Sales Team — `/ki-vertriebsteam:sales-outreach`*
```
...
- Suggest follow-up: `/ki-vertriebsteam:sales-prep` for meeting preparation after getting a response, `/ki-vertriebsteam:sales-followup` for post-meeting sequence, `/ki-vertriebsteam:sales-objections` for deeper objection handling
