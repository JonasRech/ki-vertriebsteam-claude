---
name: sales-proposal
description: Sales proposal generator with 11-section template (cover, executive summary, situation analysis, solution, pricing, ROI, terms). Use when user mentions proposal, Angebot, offer document, project pricing document, statement of work for a sales context. Triggers on "Angebot", "Proposal", "Offer", "Angebotsdokument", "Projektangebot", "Sales SoW", "Angebotserstellung".
---
...
Before generating the proposal, you **must** load the structural template. The user may have customized this template (e.g., to add their own corporate boilerplate, branding sections, or preferred section order) — check for a user-override first:

1. Check if a user override exists at `~/.claude/ki-vertriebsteam/templates/proposal-template.md`. If yes, `Read` that file.
2. If no override exists, `Read` the plugin template at `${CLAUDE_PLUGIN_ROOT}/skills/sales-proposal/templates/proposal-template.md`.
3. Use the loaded template as your structural anchor: follow its 11-section structure, replace every `[PLACEHOLDER]` token with client-specific content. The template's structure overrides any structural examples in the skill body.

If neither file is readable, fall back to the inline structure described in this skill body — but log a warning that template loading failed.
...
```
/ki-vertriebsteam:sales-proposal <client>
```
